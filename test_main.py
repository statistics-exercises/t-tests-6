...
class UnitTests(unittest.TestCase) :
    def test_testStatistic(self) : 
        for n in range(1,21) :
            ...
            self.assertTrue( np.abs( stat - testStatistic( samples, mu) )<1e-7, "your function to compute the test statistic is not working correctly" )
    ...
    def test_pvalue(self) : 
        for n in range(2,21) :
            ...
            pval = scipy.stats.t.cdf(stat,len(samples)-1)
            self.assertTrue( np.abs( pval - pvalue( samples, mu ) )<1e-7, "your function to calculate the pvalue is not working correctly" )

        stat = testStatistic( data, 2 )
        pval = scipy.stats.t.cdf(stat,len(data)-1)
        self.assertTrue( np.abs( pval - pvalue( data, 2 ) )<1e-7, "your function to calculate the pvalue is not working correctly" )
